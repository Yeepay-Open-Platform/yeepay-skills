...
- `SKILL.md`：快速通道明确「≠ 跳过易错点」；技术执行顺序步骤 2 升级为强制阅读 `## 易错点` 门禁（未完成禁止 curl/生成代码）；步骤 5～7 补充前置条件；标准输出模板 C 新增「关键易错点」必填节。
- `APP支付（使用易宝小程序）.md`、`APP支付（使用客户小程序）.md`：顶部新增易错点与接口区分警示；托管下单补充 `preventTheftMark` 默认不传说明。
...
- `SKILL.md` 面客纪律④新增「生产环境调用强约束」：生产环境实际接口调用须商户显式二次确认后方可发起，未确认默认沙箱或仅输出方案/命令。
- `通用技术纪律` 补充对应呼应条目。
