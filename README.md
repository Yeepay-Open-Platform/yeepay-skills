...
| 项 | 要求 |
| --- | --- |
| [Cursor](https://cursor.com) | 支持 Agent Skills 的桌面版 |
| Node.js（推荐安装方式） | 用于执行 `npx skills add` |
| Python（可选） | 仅在使用 `scripts/` 联调工具时需要 **≥ 3.10** |
...
适合需要本地开发、改 skill 或固定 Git 版本的场景。
...
重启 Cursor 或打开新 Agent 会话后，在对话中提及易宝/YOP 相关关键词，Agent 应自动匹配本技能。

### 3. 试试问 Agent

安装完成后，可在 Cursor Agent 中直接提问，例如：
...
面向商户接入场景的 Cursor Agent Skill。安装后，在对话中提及易宝/YOP 接入、支付、退款、分账、提现、对账或联调排障等问题时，Agent 会自动加载 [SKILL.md](./SKILL.md)，按其中的面客纪律、产品决策与文档加载协议协助完成选型、接入与排障。

Agent 的执行规则、技术纪律、路由与输出模板均在 `SKILL.md` 与 `references/` 中维护，本 README 不重复展开。

## 覆盖范围

| 业务域 | 覆盖内容 |
| --- | --- |
| 收单 | 小程序、APP、H5、公众号、被扫/主扫等 |
| 退款 | 申请、查询、回调与排障 |
| 分账 | 订单分账、余额分账、入账方 |
| 出款 | 结算、提现 |
| 对账 | 交易、分账、资金、结算对账 |
| 运维排障 | 验签、回调、错误码、沙箱、上线检查 |
