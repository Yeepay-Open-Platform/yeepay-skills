...
2. 妥善保管**商户私钥**；`account`/`balance` 出款类接口按需配置 CFCA。
3. **Java SDK**：不要在配置中手写易宝平台公钥 / SM2 平台证书——SDK 已内置平台公钥并支持证书自动更新；手配错误反而会导致验签失败。详见 `开始对接/SDK使用说明.md` §三。
4. 生产交易走商户自有系统；`scripts/` 仅用于联调验证。
