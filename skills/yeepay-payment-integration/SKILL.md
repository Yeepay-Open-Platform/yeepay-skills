...
description: >-
  易宝支付（Yeepay）开放平台接入最佳实践。涵盖入网（服务商/平台商拓展子商户进件、商户信息变更、
  沉默商户解冻、微信/支付宝实名认证）、收单（小程序支付、APP支付、浏览器H5支付、微信内H5+公众号支付、
  被扫付款码、主扫独立码/聚合码）、退款、分账（订单分账、余额分账、入账方管理）、出款（结算、提现）、
  对账全场景的产品选型、接入指导与联调排障；商户场景不明时会先澄清业务场景再推荐产品。
  当用户提到"接入易宝"、"对接易宝支付"、"yeepay"、"YOP"、"聚合支付"、"入网"、"进件"、"入件"、
  "子商户入网"、"商户报备"、"实名认证"、"支付下单"、"小程序支付"、
...
2. **【强制】阅读场景 md 与易错点**：先按下方「业务域路由」定位业务域（入网/收单/退款/分账/出款/对账），收单再按「收单场景索引」锁定具体场景文件；**完整阅读** `references/产品能力/<业务域>/<场景>.md` 全文，其中 `## 易错点` **章节为必读**（及该节引用的关联文档，如 `prePayTn唤起方式速查.md`）。**未完成本步禁止进入步骤 5（curl doc_md）及步骤 7（生成代码/参数表）**。**若映射不到任何业务域/场景**（需求超出覆盖范围或仍有歧义）→ 用 `产品决策.md` **模板 7（超出产品能力范围）** 回复或退回面客纪律①②澄清，**不臆造场景路径**。
