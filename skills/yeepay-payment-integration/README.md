...
面向易宝支付**接入、联调与排障**场景的 Agent Skill。安装后，在对话中提及「接入易宝支付」「YOP」「入网」「进件」「小程序支付」「APP 支付」「退款」「分账」「提现」「对账」「放款」「协议支付」「代扣」「签约绑卡」「信用卡还款」「航旅易达」「机票接口」「验签失败」「回调收不到」等问题时，Agent 会自动加载 [SKILL.md](./SKILL.md)，按其中的面客纪律、产品决策与文档加载协议协助完成选型、接入与排障。
