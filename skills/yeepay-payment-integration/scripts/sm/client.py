...
def _content_sm3(method: str, params: dict, json_body: str | None, files) -> str:
    payload = digest_payload_text(method, params, json_body, files)
    return sm3_hex(payload.encode("utf-8"))
...
def build_request(app_key, private_key_hex, method, path, params=None,
                  ...
                  sign_random_hex=None, content_type=None):
    """构造已签名的 SM 请求（url, headers, body / multipart）。"""
    method = method.upper()
    params = params or {}
    ...
    content_sm3 = _content_sm3(method, params, json_body, file_parts)
...
def _verify_response(resp, verify, yop_pubkey, cert_dir, strict_verify):
    if not verify:
        return resp
    import os
...
def call(app_key, private_key_hex, method, path, params=None, json_body=None,
         ...
         yop_pubkey=None, cert_dir=None, strict_verify=False, content_type=None):
    """发送已签名请求。"""
    if base_url is None:
        base_url = resolve_gateway(sandbox=sandbox)
    req = build_request(
        app_key, private_key_hex, method, path, params, json_body, files,
        base_url, content_type=content_type,
    )
    resp = send_request(req, timeout=timeout)
    return _verify_response(resp, verify, yop_pubkey, cert_dir, strict_verify)


def upload(app_key, private_key_hex, path, params=None, files=None,
           base_url=None, sandbox=False, timeout=120, verify=False, yop_pubkey=None,
           cert_dir=None, strict_verify=False):
    """multipart 文件上传（POST）。生产默认 yos 网关；沙箱走 sandbox 域名。"""
    if base_url is None:
        base_url = resolve_gateway(sandbox=sandbox, yos=True)
    return call(
        app_key, private_key_hex, "POST", path, params=params, files=files,
        base_url=base_url, sandbox=sandbox, timeout=timeout, verify=verify,
        yop_pubkey=yop_pubkey, cert_dir=cert_dir, strict_verify=strict_verify,
    )


def download(app_key, private_key_hex, path, params=None, base_url=None,
             sandbox=False, timeout=120, save_path=None, verify=False,
             yop_pubkey=None, cert_dir=None, strict_verify=False):
    """文件下载（GET）。生产默认 yos 网关；沙箱走 sandbox 域名。"""
    if base_url is None:
        base_url = resolve_gateway(sandbox=sandbox, yos=True)
    req = build_request(
        app_key, private_key_hex, "GET", path, params=params, base_url=base_url,
    )
    resp = send_request(req, timeout=timeout, stream=True)
    _verify_response(resp, verify, yop_pubkey, cert_dir, strict_verify)
    if save_path:
